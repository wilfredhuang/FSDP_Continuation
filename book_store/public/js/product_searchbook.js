...
function getBook() {
	...
	fetch(
		"https://www.googleapis.com/books/v1/volumes?q=" +
			title +
			"&maxResults=1&AIzaSyA6Q9oqGie8vr5XgkvlTkhFvufujR2UPOk"
	)
		.then((res) => {
			return res.json();
		})
      ...
      */
			...
			details.value = JSON.stringify(
				...
				1,
				JSON.stringify(book["items"][0]["volumeInfo"]["description"], null, 2)
					.length - 1
			);
			if (
				JSON.stringify(book["items"][0]["saleInfo"]["saleability"], null, 2) ==
				'"FOR_SALE"'
			) {
				price.value = JSON.stringify(
					book["items"][0]["saleInfo"]["listPrice"]["amount"],
					null,
					2
				);
			}
			product_image.value = JSON.stringify(
