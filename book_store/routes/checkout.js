			...
			if (userCart.length < 1) {
				...
				for (z in userCart) {
					if (z == id) {
						...
						userCart[z].SubtotalPrice = (
							parseFloat(userCart[z].SubtotalPrice) + parseFloat(product.price)
						).toFixed(2);
						check = true;
						console.log(userCart);
					}
...
router.get("/cart", (req, res) => {
	...
	for (z in userCart) {
		userCart[z].SubtotalPrice = (
			userCart[z].Quantity * userCart[z].Price
		).toFixed(2);
	}
	...
	if (coupon_type == "OVERALL") {
		...
		if (
			parseFloat(shipping_discounted_price) >
			parseFloat(shipping_discount_limit)
		) {
			discounted_price = shipping_discount_limit;
			shipping_fee = (
				parseFloat(shipping_fee) - parseFloat(discount_limit)
			).toFixed(2);
			full_total_price = (
				parseFloat(full_subtotal_price) + parseFloat(shipping_fee)
			).toFixed(2);
			...
			discounted_price = (
				parseFloat(shipping_fee) * parseFloat(shipping_discount)
			).toFixed(2);
			shipping_fee = (
				parseFloat(shipping_fee) *
				(1 - parseFloat(shipping_discount))
			).toFixed(2);
			full_total_price = (
				parseFloat(full_subtotal_price) + parseFloat(shipping_fee)
			).toFixed(2);
		...
		if (parseFloat(sub_discounted_price) > parseFloat(sub_discount_limit)) {
			...
			console.log(full_subtotal_price);
			full_subtotal_price = (
				parseFloat(full_subtotal_price) - parseFloat(sub_discount_limit)
			).toFixed(2);
			console.log(full_subtotal_price);
			full_total_price = (
				parseFloat(full_subtotal_price) + parseFloat(shipping_fee)
			...
			discounted_price = (
				parseFloat(full_subtotal_price) * parseFloat(sub_discount)
			).toFixed(2);
			full_subtotal_price = (
				parseFloat(full_subtotal_price) * parseFloat(1 - sub_discount)
			).toFixed(2);
			full_total_price = (
				parseFloat(full_subtotal_price) + parseFloat(shipping_fee)
			).toFixed(2);
		...
		.then((coupon) => {
			...
			if (coupon_type == "OVERALL") {
				...
				alertMessage(
					res,
					"success",
					`${coupon.discount * 100}% off your total shipping fee (save up to $${
						coupon.limit
					})`,
					"fas fa-exclamation-circle",
					true
...
router.get("/delete/:id", (req, res) => {
	...
	alertMessage(
		res,
		"success",
		req.params.id + " is successfully deleted",
		"fas fa-sign-in-alt",
		true
	);
