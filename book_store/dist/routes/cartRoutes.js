...
router.get("/view-order-details/:id", ensureAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const { progressPercentage, progressColour, progressColourText, deliveryStatusResult } = statusMapping[deliveryStatus] || statusMapping.default;
...
// ------------------------------------------------------------
router.get("/view-order-details-admin/:id", ensureAuthenticated, ensureAdminAuthenticated, async (req, res) => {
...
// 6. Display label URL
// ------------------------------------------------------------
router.get("/displayLabelUrl/:id", ensureAuthenticated, ensureAdminAuthenticated, async (req, res) => {
    try {
        const shippingId = req.params.id;
        if (!shippingId)
            return res.status(400).send("Missing shippingId");
...
// 7. Print Label PDF
// ------------------------------------------------------------
router.get("/printLabelPDF/:id", ensureAuthenticated, ensureAdminAuthenticated, async (req, res) => {
    try {
        const shippingId = req.params.id;
        if (!shippingId)
            return res.status(400).send("Missing shippingId");
...
// ------------------------------------------------------------
router.get("/check-delivery", (_req, res) => {
...
router.post("/checkingDelivery", async (req, res) => {
    ...
    try {
        ...
        const [progressPercentage, progressColour, progressColourText, deliveryStatusResult] = mapping[deliveryStatus] ?? [0, "bg-dark", "text-dark", "Unknown"];
