...
export const checkEmptyCart = (userCart: Record<string, any>): boolean => {
  for (const id in userCart) {
    if (Object.prototype.hasOwnProperty.call(userCart, id)) return false;
  }
  return true;
};

export const countCartQty = (userCart: Record<string, any>): number => {
  let total = 0;
  for (const id in userCart) {
...
export const checkPromo = (couponObj: any): boolean => couponObj != null;

export const convertDiscount = (discount: number): number => discount * 100;

export const displayCouponType = (type: string): string => {
  switch (type) {
    case "OVERALL":
      return "total order";
    case "SHIP":
...
export const checkShipmentCountrySingapore = (country?: string): boolean => {
...
export const updateCartItem = (
  ...
  if (discount) {
    ...
    const { discountedSubtotalPrice } = calculateDiscountedPrice(
      req,
      qty,
      price,
      rate,
...
export const refreshCartCalculations = async (
  ...
  // 🧾 Final totals
  const grandTotal =
    initialSubtotal - discountSavingsTotal - couponSavingsTotal;
